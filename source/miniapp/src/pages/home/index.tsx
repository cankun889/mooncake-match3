...
function ValueRow({ dp }: { dp: SchemaItem }) {
  ...
  const isVolume  = /(volume|sound)/.test(String(dp.code).toLowerCase());
  const unit = property.unit || (isBattery ? '%' : '');
  const accentColor = isBattery ? '#10b981' : isVolume ? '#6366f1' : 'var(--app-M4, #ff7a45)';
  const pct = max > min ? Math.max(0, Math.min(100, Math.round(((num - min) / (max - min)) * 100))) : 0;
  const fillBg = isBattery
  ...
  return (
    <View className={styles.sliderCard}>
      <View className={styles.sliderHeader}>
        ...
        <View className={`${styles.sliderValueWrap} ${isVolume ? styles.sliderValueWrapVolume : ''} ${isBattery ? styles.sliderValueWrapBattery : ''}`}>
          <Text className={`${styles.sliderValueBig} ${isBattery ? styles.sliderValueBattery : ''} ${isVolume ? styles.sliderValueVolume : ''}`}>{num}</Text>
          {!!unit && <Text className={`${styles.sliderValueUnit} ${isBattery ? styles.sliderValueBatteryUnit : ''} ${isVolume ? styles.sliderValueVolumeUnit : ''}`}>{unit}</Text>}
        </View>
      </View>
        ...
        {writable ? (
          <>
            ...
            <View className={styles.sliderTrack}>
              <Slider
                ...
                maxTrackColor="var(--app-B2, #e2e8f0)"
                minTrackColor={accentColor}
                maxTrackHeight={isVolume ? '24px' : '20px'}
                minTrackHeight={isVolume ? '24px' : '20px'}
                thumbColor="#ffffff"
                thumbWidth={isVolume ? '30px' : '36px'}
                thumbHeight={isVolume ? '30px' : '36px'}
                thumbBoxShadowStyle="0px 4px 12px rgba(0,0,0,0.16)"
                onChange={(e: any) => {
          ...
          <View className={styles.progressTrack}>
            <View
              ...
              style={{ width: `${pct}%`, background: fillBg } as any}
            />
          </View>
        )}
      </View>
      <View className={`${styles.sliderRange} ${!writable ? styles.sliderRangeFull : ''}`}>
